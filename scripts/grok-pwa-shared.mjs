...
export const DEFAULT_APP_NAME = "LastCommit";
...
const APP_SHARE_META_KEYS = new Set([
  "og:description",
  "og:image",
  "og:image:width",
  "og:image:height",
...
/** Platform chrome is only appropriate on the builder's own hosts. */
...
export function grokOgHeadTags({
  ...
  if (publicHost) {
    ...
    if (custom || isGrokPlatformHost(publicHost)) {
      let image = custom
...
function metaKey(tag) {
  const match = tag.match(/\b(?:property|name)\s*=\s*["']([^"']+)["']/i);
  return match ? String(match[1]).toLowerCase() : "";
}

function appShareMetaTags(html) {
  return [...String(html).matchAll(/<meta\b[^>]*>/gi)]
    .map(([tag]) => tag)
    .filter((tag) => APP_SHARE_META_KEYS.has(metaKey(tag)));
}
...
export function injectGrokPwaHead(html, ctx = {}) {
  ...
  const platformHost = isGrokPlatformHost(host);
  ...
  const generatedShareTags = grokOgHeadTags({ host, appName, site, documentTitle, cwd });
  const generatedShareKeys = new Set(generatedShareTags.map(metaKey));
  const appTags = platformHost
    ? []
    : appShareMetaTags(html).filter((tag) => !generatedShareKeys.has(metaKey(tag)));
  let next = stripShareMetaTags(html);

  const missing = (platformHost ? grokPwaHeadTags(appName) : [])
    .filter(([key]) => {
  ...
  next = insertAfterHeadOpen(next, [...generatedShareTags, ...appTags].join(""));
  ...
  const creatorTags = platformHost ? grokXCreatorHeadTags(creator, creatorId) : [];
