...
test("104.x is public", () => assert.equal(isPrivateHost("104.16.1.1"), false));
test("10.x is private", () => assert.equal(isPrivateHost("10.0.0.1"), true));
test("172.16-31 is private", () => {
  assert.equal(isPrivateHost("172.16.0.1"), true);
  assert.equal(isPrivateHost("172.15.0.1"), false);
});
test("192.168 is private", () => assert.equal(isPrivateHost("192.168.1.1"), true));
