...
test("custom product keeps route-specific share metadata", () => {
  const html = '<html><head><title>LastCommit Proof #5</title><meta property="og:description" content="Proof of Abandonment"><meta property="og:url" content="https://lastcommit.bydx.fun/proof/5"><meta property="og:type" content="article"><meta property="og:image" content="https://lastcommit.bydx.fun/og.svg"><meta name="twitter:title" content="LastCommit Proof #5"><meta name="twitter:description" content="Proof of Abandonment"></head></html>';
  const out = injectGrokPwaHead(html, { host: "lastcommit.bydx.fun" });
  assert.match(out, /property="og:description" content="Proof of Abandonment"/);
  assert.match(out, /property="og:url" content="https:\/\/lastcommit\.bydx\.fun\/proof\/5"/);
  assert.match(out, /name="twitter:title" content="LastCommit Proof #5"/);
  assert.match(out, /name="twitter:description" content="Proof of Abandonment"/);
  assert.equal(out.split('name="twitter:card"').length - 1, 1);
});
...
test("site title LastCommit is a real name, not a sentinel", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    host: "wild-race.grok.me",
    site: { title: "LastCommit" },
  });
  assert.match(out, /property="og:title" content="LastCommit"/);
});

test("custom domains omit builder chrome and placeholder cards", () => {
  assert.equal(isGrokPlatformHost("lastcommit.bydx.fun"), false);
  const out = injectGrokPwaHead(
    "<html><head><title>LastCommit</title></head></html>",
    { host: "lastcommit.bydx.fun", appName: "LastCommit", projectId: "proj-123" },
  );
  assert.match(out, /property="og:title" content="LastCommit"/);
  assert.doesNotMatch(out, /grok|xai|x\.ai|__grok/i);
});
...
test("emits og:image for a public host and prefers a custom card", () => {
  const empty = mkdtempSync(join(tmpdir(), "grok-pwa-og-"));
...
test("streaming injector handles </head> split across chunks", () => {
  const injector = createHeadInjector({ appName: "Wild Race", host: "wild-race.grok.me" });
  const chunks = [
    ...injector.push("<html><head><title>x</title></he"),
    ...injector.push("ad><body>hello</body></html>"),
  ];
...
test("streaming injector falls back when no </head> is seen", () => {
  const injector = createHeadInjector({ host: "wild-race.grok.me" });
...
test("names the install page from host slug", () => {
  assert.equal(appNameFromHost("localhost:8080"), "LastCommit");
  assert.equal(appNameFromHost("172.17.154.217:8080"), "LastCommit");
  assert.equal(appNameFromHost("wild-race.grok.me"), "Wild Race");
});

test("rejects hosts that are not plain slugs", () => {
  assert.equal(appNameFromHost("<script>alert(1)</script>"), "LastCommit");
  assert.equal(appNameFromHost('"><img src=x onerror=1>.grok.me'), "LastCommit");
});
